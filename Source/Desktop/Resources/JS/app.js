...
class App 
{
  ...
  constructor() 
  {
    this.#errors = 
    {
      ...
      backgroundColorInvalidError: 'App Error: Invalid color value for color.',
      backgroundColorTypeError: 'App Error: Expected type string for color.',
      ...
      filePathTypeError: 'App Error: Expected type string for filePath.',
      filePathsTypeError: 'App Error: Expected type array for filePaths.',
      ...
      scriptFailedToLoadError: 'App Error: Failed to load script at path: ',
  ...
  /**
   * Public method to dynamically load extra JavaScript files into the HTML document sequentially.
   * This ensures that all scripts are loaded in the order they are provided.
   *
   * @param {string[]} filePaths - An array of file paths for the JavaScript files to load.
   * @returns {Promise<void>} A promise that resolves when all scripts are loaded.
   */
  async loadScripts({ root = "", filePaths = [] } = {}) 
  {
    if(!typechecker.check({ type: 'array', value: filePaths })) 
    {
      console.error(this.#errors.filePathsTypeError);
      return;
    }

    for(let filePath of filePaths) 
    {
      if(!typechecker.check({ type: 'string', value: filePath })) 
      {
        console.error(this.#errors.filePathTypeError);
        continue;
      }
      ...
      await new Promise((resolve, reject) => 
      {
        ...
        script.src = root + filePath;
        script.type = 'text/javascript';

        script.onload = () => { resolve(); };
        script.onerror = () => 
        {
          let error = this.#errors.scriptFailedToLoadError + `${filePath}`
          console.error(error);
  ...
  /**
   * Public method to dynamically load extra CSS files into the HTML document sequentially.
   * This ensures that all styles are loaded in the order they are provided.
   *
   * @param {string[]} filePaths - An array of file paths for the CSS files to load.
   * @returns {Promise<void>} A promise that resolves when all styles are loaded.
   */
  async loadStyles({ root = "", filePaths = [] } = {}) 
  {
    if(!typechecker.check({ type: 'array', value: filePaths })) 
    {
      console.error(this.#errors.filePathsTypeError);
    ...
    for(let filePath of filePaths) 
    {
      ...
      await new Promise((resolve, reject) => 
      {
        ...
        link.onerror = () => 
        {
          const error = this.#errors.scriptFailedToLoadError + `${filePath}`;
          console.error(error);
          reject(new Error(error));
        };
  ...
  /** Private internal method to init the app info storage object and do initial business logic on app startup. */
  _initAppInfo()
  {
    let appInfo = Lockr.get(this.#appInfoStorageKey, {});
